            ...
            for swing_point in swing_point_query:
                ...
                if swing_point_counter == 1:
                    if swing_point.label == 'LH':
                        last_sp = swing_point
                        last_sp_price = last_sp.price
                        logger.error(f'Detected first LH swingpoint. Price: {last_sp_price}. Latest close price: {latest_price.close_price}')
                        recent_swing_points.append(swing_point)
                    else:
                        # This strategy cannot be true. End review of swing points.
                    ...
                    if swing_point.label == 'LL' and most_recent_label == 'LH':
                        ...
                        logger.error(f'Found a prior LL. Price: {trough_sp_price}.')
                        recent_swing_points.append(swing_point)
                    else:
                        # This strategy cannot be true. End review of swing points.
                    ...
                    if swing_point.label == 'LH' and most_recent_label == 'LL':
                        ...
                        logger.error(f'Found a prior LH. prior_lh_price: {prior_lh_price}. So swing points match the strategy.')
                    else:
                        logger.error(f'Third swingpoint not LH. Strategy not valid.')
                    break
                swing_point_counter += 1
        ...
        try:
            ...
            if len(T_prev) > 0:
                ...
                data = {'latest_T': str(latest_T), 'T_prev': str(T_prev), 'max_T': str(max(T_prev)), 'count_T_prev': str(len(T_prev)),
                        'prior_trend_duration' : str(prior_trend_duration), 'recent_swing_points' : recent_swing_points,
                        'final_upswing_size' : str(final_upswing_size), 'duration_after_latest_sp' : str(duration_after_latest_sp)} # recent_swing_points not as a string as it gets removed and accessed if present.
                        ...
                        if prev_low_price is None or price.low_price > prev_low_price:
                            ...
                            if len(pattern) == 3:
                                ...
                                individual_candles[ind_candle_count] = {'datetime': price.datetime.isoformat(),
                                                                        'low_price': float(price.low_price), 'high_price': float(price.high_price)}
                                ind_candle_count += 1
                            elif len(pattern) < 3:
                    ...
                    if ll_price_exceeded == False and price.low_price < most_recent_ll_price:
                        ...
                        if prev_high_price is None or price.high_price < prev_high_price:
                            ...
                            if len(pattern) < 3:
                                ...
                                pattern_detected = True
                                individual_candles[ind_candle_count] = {'datetime' : price.datetime.isoformat(), 'low_price': float(price.low_price), 'high_price' : float(price.high_price)}
                                ind_candle_count += 1
                            if len(pattern) == 0:
                                pattern = ['lower']
                            ...
                            pattern = []
                            prev_high_price = price.high_price
                    new_start = {'datetime' : price.datetime.isoformat(), 'low_price':float( price.low_price), 'high_price' : float(price.high_price)}
            if sections > 0 and pattern_detected == True:
                logger.error(f'Strategy confirmed to be valid.')
                action_buy = True
